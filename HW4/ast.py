...
class Type:
    ...
    def __init__(self, basetype, params=None):
        if ((params == None) or (params == 0)):
            ...
            elif (classtable.has_key(basetype)):
...
class Method:
    ...
    def printout(self):
        ...
        print ', '.join(["%d"%i.id for i in self.vars.get_params()])
        self.vars.printout()
        print "Method Body:"
        self.body.printout()
...
class ReturnStmt(Stmt):
    def __init__(self, expr, rtype, lines):
        self.lines = lines
        self.expr = expr
        self.rtype = rtype
...
class ConstantExpr(Expr):
    def __init__(self, kind, arg=None, lines=None):
        ...
        if(kind == 'true' or kind == 'false') :
            self.basetype = Type('boolean')
        else :
            self.basetype = Type(kind)
        if (kind=='int'):
            self.int = arg
        elif (kind == 'float'):
            self.float = arg
...
class AssignExpr(Expr):
    def __init__(self, lhs, rhs, lines):
        ...
        self.basetype = None
    def __repr__(self):
        return "Assign({0}, {1}, {2}, {3})".format(self.lhs, self.rhs, self.lhs.basetype, self.rhs.basetype)
...
class FieldAccessExpr(Expr):                                                                                #need
...
class MethodInvocationExpr(Expr):                                                                          #need
