...
def p_method_decl_header_void(p):
    ...
    global current_context, current_vartable, current_method
...
def p_stmt_return(p):
    ...
    if(current_context != 'method') :
        p[0] = ast.ReturnStmt(p[2], ast.Type('void'), p.lineno(1))
    else :
        p[0] = ast.ReturnStmt(p[2], current_method.rtype, p.lineno(1))
...
def p_primary_this(p):
    ...
    p[0].basetype = ast.Type(current_class)
def p_primary_super(p):
    'primary : SUPER'
    p[0] = ast.SuperExpr(p.lineno(1))
    if(current_class.superclass == None) :
        p[0].basetype = ast.Type('Null')
    else :
        p[0].basetype = ast.Type(current_class.superclass)
...
def p_field_access_id(p):
    ...
    if (v != None):
        ...
        if (c != None):
            ...
            t = ast.ThisExpr(p.lineno(1))
            t.basetype = ast.Type(current_class)
            p[0] = ast.FieldAccessExpr(t, vname, p.lineno(1))
...
#     if (not isinstance(p[0].basetype, ast.Type)) or (p[0].basetype.kind != 'basic') or (p[0].basetype.typename != 'int' and p[0].basetype.typename != 'float') :
#         decaflexer.errorflag = True
#         p[0].basetype = ast.Type('error')
def p_expr_unop_not(p):
    'expr : NOT expr'
    p[0] = ast.UnaryExpr('neg', p[2], p.lineno(1))
#     if (not isinstance(p[0].basetype, ast.Type)) or (p[0].basetype.kind != 'basic') or (p[0].basetype.typename != 'boolean') :
#         decaflexer.errorflag = True
#         p[0].basetype = ast.Type('error')
...
#     if(not typecheck.isSubClass(p[3], p[1])):
#         p[0].basetype = ast.Type('error')
#         decaflexer.errorflag = True
